  ...
  return (
    <div className="form-card" style={{ marginBottom: '2rem' }}>
      ...
      <form onSubmit={handleSubmit} className="productos-form">
        ...
        {/* Bloque 2: Costeo en Dólares (Diferenciado y Resaltado) */}
        <div 
          style={{ 
            marginTop: '1.5rem', 
            padding: '1.35rem', 
        ...
        >
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Truck size={16} /> 2. Costeo en Dólares (USD por unidad)
            </span>
            <span style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>
              Conversión: <strong style={{ color: 'var(--color-text-heading)' }}>${Number(cotizacionDolar || 0).toLocaleString('es-AR')} ARS</strong>
            </span>
          </div>

          <div className="form-row" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
            <div className="form-group">
              <label style={{ color: '#F3F5F7', fontWeight: 600 }}>Costo Origen (USD)</label>
              <input 
                type="number" 
                step="0.01" 
                min="0" 
                name="costo_origen" 
                placeholder="Ej: 2.00" 
                value={formData.costo_origen} 
                onChange={handleChange} 
                required 
                style={{ background: 'var(--color-bg-card)', borderColor: 'rgba(201, 162, 39, 0.25)' }}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input 
                ...
                step="0.01" 
                min="0" 
                name="impuestos_aduana" 
                placeholder="0.00" 
                value={formData.impuestos_aduana} 
                onChange={handleChange} 
                style={{ background: 'var(--color-bg-card)' }}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input 
                ...
                placeholder="0.00" 
                value={formData.nacionalizacion} 
                onChange={handleChange} 
                style={{ background: 'var(--color-bg-card)' }}
              />
            </div>
          ...
          >
            <div style={{ fontSize: '0.85rem', color: 'var(--color-text-heading)' }}>
              Subtotal Costo: <strong style={{ color: '#F3F5F7' }}>USD ${costoTotalUSD.toFixed(2)}</strong>
            </div>
            <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--color-accent)', textTransform: 'uppercase' }}>
              Costo Real en Depósito (ARS): <strong style={{ fontSize: '1.3rem', marginLeft: '6px', color: '#DFBA45' }}>{formatPrice(costoTotalARS)}</strong>
            </div>
          </div>
        </div>
